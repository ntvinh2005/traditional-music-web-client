  ...
  return (
    <>
              ...
              {index % 2 === 0 ? (
                <div className="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6 m-0">
                  <dt className="text-sm font-medium">
                    <Link to={"/post/" + post._id}>Bài học số {index + 1}</Link>
                  </dt>
      ...
      <div className="shadow place-self-center m-5 rounded-full">
        <Link to={"/course/createLessons/" + courseId} className="items-center justify-center rounded-full border border-transparent bg-yellow-600 px-8 py-3 text-base font-medium text-white hover:bg-yellow-700 md:py-4 md:px-10 md:text-lg hover:text-white">Tạo bài học mới</Link>
      </div>
    </>
  );
};
